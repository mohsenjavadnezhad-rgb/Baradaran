<?php
/* گام ۳ از ۴ (سبد → بررسی عکس → بررسی موجودی → ثبت سفارش): «در انتظار
   بررسی موجودی». مشتری فقط پس از یک اقدام در part-check.php (آپلود یا رد
   کردن) به اینجا می‌رسد — این صفحه خودش هیچ اقدامی نمی‌گیرد، فقط وضعیت
   ردیف part_checks را نشان می‌دهد و صبر می‌کند.
   ۲۰۲۶-۰۸-۳۰: پیش‌تر این حالت‌ها داخل part-check.php رندر می‌شدند؛ به
   خواست کاربر («امکان آپلود عکس رو بذار توی یک صفحه، صفحه سوم بررسی
   موجودی میشه») این‌جا صفحهٔ مستقل خودش را گرفت. */
...
/* بدون ردیف منطبق، یا ردیفی که رد شده: اینجا معنا ندارد — برگرد به گام ۲
...
<div class="container">
    ...
    <?php /* گام ۱ و ۲ قابل کلیک‌اند تا مشتری بتواند عقب برود (مثلا برای
            فرستادن عکس تازه به part-check.php)، هم‌الگوی همان صفحه. */ ?>
    <ol class="pchk-steps">
    ...
    <?php /* ---------- در انتظار بررسی موجودی ----------
            کادر بزرگ مسدودکننده: خواستهٔ کاربر ۲۰۲۶-۰۸-۳۰. تا وقتی ادمین در
            پنل تیک «موجودی کالا را تأیید می‌کنم» را نزند (همراه با تأیید
            مطابقت عکس، اگر عکسی بوده)، مشتری همین‌جا می‌ماند — نه دکمهٔ فرار،
            نه timeout. صفحه هر چند ثانیه خودش تازه می‌شود تا با تأیید ادمین،
            بدون کاری از سمت مشتری به تسویه‌حساب برود. */ ?>
    ...
    <div class="pchk-panel is-wait pchk-panel-big">
        ...
        <p class="pchk-panel-text" style="text-align:center;">
            ...
            کارشناس ما مطابقت عکس قطعه را تأیید کرد؛ فقط <b>تأیید موجودی انبار</b> باقی مانده.
            <?php else: ?>
            درخواست شما رسید و در نوبت <b>بررسی موجودی انبار</b> است.
            <?php endif; ?>
            به‌محض تأیید کارشناس ما، همین صفحه خودکار سبز می‌شود و مستقیم به
            <b>ثبت سفارش و پرداخت</b> می‌روید — کاری لازم نیست بکنید، فقط این صفحه را باز نگه دارید.
        </p>
    ...
    <div class="pchk-panel is-ok pchk-panel-big pchk-panel-pop">
        ...
        <?php require __DIR__ . '/includes/partcheck-photos.php'; ?>
        <div class="pchk-actions" style="justify-content:center;">
            <a href="checkout.php" class="btn btn-primary btn-lg"><?= icon('cart') ?>ادامه به ثبت سفارش و پرداخت</a>
            <a href="cart.php" class="btn btn-secondary"><?= icon('arrow-right') ?>بازگشت به سبد خرید</a>
        </div>
    </div>
    <?php /* خودکار وارد تسویه‌حساب نمی‌شویم — خواستهٔ کاربر: فقط خود تغییر
            وضعیت (تیک سبز) باید بدون رفرش دستی دیده شود؛ رفتن به صفحهٔ
            پرداخت را خود مشتری با همین دکمهٔ بالا می‌زند. */ ?>
    <?php endif; ?>
</div>
