...
/* ==========================================================================
   ...
   «مدل خودرو» (زیرمجموعهٔ همان برند در سیستم دستهٔ برند/مدل — همان
   ...
   همان لحظه که برند انتخاب شده (بدون انتخاب مدل/سال) همه محصولات آن برند
   دیده می‌شوند؛ چیپ‌های «همهٔ مدل‌ها»/«همه سال‌ها» هم برای برگشتن از یک
   انتخاب قبلی به همین حالت‌اند. محصولی که سالی برایش ثبت نشده («از/تا»
...
/* ۲۰۲۶-۰۹-۰۳: خاموش/روشن‌کردن مرحلهٔ برند/مدل از پنل (تنظیمات ← سال تولید
   خودرو، همان بخش کنترل مراحل). خاموش‌بودن برند یعنی این دسته اصلاً
   برند نمی‌خواهد — محصولاتش مستقیم نشان داده می‌شوند، بدون گیت. */
$brandStepOn = partsBrandStepEnabled();
$modelStepOn = partsModelStepEnabled();
if (!$brandStepOn) $brandId = 0; /* برند انتخابی (اگر در URL بود) نادیده گرفته می‌شود */
...
/* مدل زیرمجموعهٔ همین برند (خواستهٔ کاربر: «وقتی پژو رو انتخاب می‌کنم
...
function renderBrandYearStep($allBrands, $selectedBrand, $brandId, $modelId, $subModels, $modelStepOn, $year, $yearOptions, $baseQs) {
    /* لینک‌های چیپ مدل/سال باید همیشه انتخاب دیگری (سال وقتی مدل عوض
       می‌شود، مدل وقتی سال عوض می‌شود) را نگه دارند — وگرنه انتخاب یکی،
       آن یکی را بی‌دلیل پاک می‌کرد. */
    $yearQs  = $year  ? '&year=' . (int)$year   : '';
    $modelQs = $modelId ? '&model=' . (int)$modelId : '';
    /* لنگر مقصد چیپ مدل: اگر سال هم فعال است، اول برو روی کادر سال
       (خواستهٔ کاربر: «بعد از کلیک، بره روی سال تولید اگه فعال بود، بعد
       بره روی محصول») — یعنی مرحلهٔ بعدی منطقی، نه رد شدن از رویش. */
    ...
    <div class="pby-box">
        ...
        <div class="pby-selrow">
            ...
            <?php /* انتخابگر مدل — زیرمجموعهٔ همان برند (خواستهٔ کاربر: بعد از
                    انتخاب برند، مدل‌هایش نشان داده شود، مثلاً پژو ۲۰۰۸/۲۰۶/...).
                    اختیاری است، دقیقاً هم‌قاعدهٔ سال: چیپ «همهٔ مدل‌ها» همیشه
                    اول است، هم برای انتخاب صریح هم برای برگشتن از یک مدل
                    انتخاب‌شده. سال (اگر انتخاب شده) در همهٔ لینک‌ها حفظ می‌شود. */ ?>
            <?php /* لنگر کلیک مدل (خواستهٔ کاربر: «بره روی سال تولید اگه فعال
                    بود، بعد بره روی محصول») — $modelAnchor بالاتر تعیین شده:
                    اگر چیپ‌های سال هم هستند مقصد کادر سال است، وگرنه مستقیم
                    خود محصولات. لینک ساده است، بدون جاوااسکریپت هم کار می‌کند. */ ?>
            <?php /* خواستهٔ کاربر: «کلید همه مدل‌ها … متفاوت باشه و بالاتر از
                    بقیه توی کادر خودش باشه» + «مدل کلید هم بیضی نباشه» —
                    «همهٔ مدل‌ها» از ردیف چیپ‌ها بیرون آمد (ردیف مجزای
                    خودش، بالای بقیه)، و چیپ‌های خود مدل‌ها کلاس جداگانه
                    گرفتند (‎.pby-modelchip‎، مستطیل گردگوشه — نه بیضی
            ...
            <?php /* لنگر کلیک سال: برخلاف مدل، بعد از سال دیگر جایی جز
    ...
    if (!$brandStepOn) {
        ...
        if (!$products) {
            /* اگر محصولی مستقیما به این زیرشاخه وصل نشده بود، جست‌وجوی نامی
               ...
               یک قاعدهٔ یکسان داشته باشند). فیلتر مدل عمداً در این مسیر
               جایگزین (جست‌وجوی نامی) نمی‌آید — «category_id» روی جدول
