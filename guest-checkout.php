...
/* «خرید بدون ثبت‌نام» — پنل مدیریت ← تنظیمات ← ثبت سفارش/ورود.
   مشتری واردنشده به‌جای صفحهٔ ورود (که کد تأیید/انتخاب نوع حساب می‌خواهد)
   فقط شمارهٔ موبایلش را اینجا می‌گذارد؛ بدون کد تأیید، بی‌صدا برایش حساب
   ساخته/پیدا و واردش می‌کنیم (همان الگوی findOrCreateCustomer+loginCustomer
   که در login.php برای حالت «پیامک خاموش» استفاده می‌شود) و به همان مسیری
   می‌رود که cart.php بدون این قابلیت مستقیم می‌فرستاد. */
...
/* مقصد بعدی فقط از میان مسیرهای شناخته‌شده پذیرفته می‌شود — نه هر آدرسی که
   در URL بیاید — تا این صفحه به‌عنوان ریدایرکت باز سوءاستفاده نشود. */
...
<div class="container">
  <div class="auth-card">
    <h1 class="auth-title"><?= icon('bag') ?>ادامهٔ خرید بدون ثبت‌نام</h1>
    <p class="auth-sub">فقط شمارهٔ موبایلتان را بگذارید — نه کد تأیید لازم است و نه ثبت‌نام جداگانه‌ای؛ مستقیم به مراحل بعدی می‌روید.</p>
