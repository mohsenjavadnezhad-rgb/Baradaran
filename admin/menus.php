...
if (!menuReady()) {
    ...
    ?>
    <div class="flash flash-error">جدولِ منوها هنوز روی این نصب ساخته نشده. یک‌بار آدرسِ <code>migrate-menus.php</code> را در مرورگر باز کنید تا ساخته و با چیدمانِ فعلیِ سایت پر شود؛ بعدش همین صفحه فعال می‌شود.</div>
    <?php
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save'])) {
    ...
    if ($label === '') {
        ...
        /* آیتم‌های سیستمی (مثلِ مگامنوی فروشگاه یا حساب‌کاربری/ورود) آدرس یا
           محتوای‌شان در کدِ سایت ساخته می‌شود — فرم اجازه ندارد آدرسِ دلخواه
           رویشان بنویسد، فقط برچسب/آیکون‌شان قابلِ تغییر است. */
...
if (isset($_GET['toggle'])) {
    ...
    $msg = 'وضعیتِ فعال‌بودن تغییر کرد.';
}
...
if (isset($_GET['delete'])) {
    ...
    if (!$row) {
        ...
        $msg = 'این آیتم سیستمی است (آدرس/محتوایش به کدِ سایت وابسته است) و حذف نمی‌شود؛ فقط می‌توانید غیرفعالش کنید.';
        $msgErr = true;
    } else {
...
/* جابه‌جاییِ ترتیب: مبادلهٔ sort_order با نزدیک‌ترین همسایهٔ همان گروه */
...
<div class="cust-tabs" style="margin-bottom:1rem;">
  ...
  <a href="?group=footer" class="cust-tab <?= $group === 'footer' ? 'active' : '' ?>"><?= icon('layers', 'ic-sm') ?> دسترسیِ سریعِ فوتر</a>
</div>

<div style="display:grid; grid-template-columns:320px 1fr; gap:1rem; margin-bottom:1rem;">
<div>
  <div class="dg-box">
    <div class="dg-box-hd"><h3><?= $editItem ? 'ویرایشِ «' . h($editItem['label']) . '»' : 'آیتمِ جدید' ?></h3></div>
    <div class="dg-box-bd" style="padding:1rem;">
      <form method="POST">
        ...
        <p style="font-size:0.75rem;color:var(--text-muted);background:var(--bg-secondary);border:1px solid var(--border-color);border-radius:var(--radius-sm);padding:0.5rem 0.7rem;margin-bottom:0.75rem;">
          <?= icon('info', 'ic-sm') ?> این آیتم سیستمی است؛ آدرس/محتوایش (مثلِ مگامنوی فروشگاه یا لینکِ حساب‌کاربری) در کدِ سایت ساخته می‌شود. فقط برچسب، آیکون، ترتیب و فعال‌بودنش قابلِ تغییر است — حذفِ واقعی ندارد.
        </p>
        <?php endif; ?>
        ...
        <div class="form-group">
          <label>آدرس *</label>
          <input type="text" name="url" class="form-control" dir="ltr" value="<?= h($editItem['url'] ?? '') ?>" placeholder="مثلاً search.php?tag=x یا https://...">
          <small style="display:block;color:var(--text-muted);font-size:0.72rem;margin-top:0.25rem;">آدرسِ داخلیِ سایت (مثل shop.php) یا یک لینکِ کاملِ خارجی.</small>
        </div>
        <?php endif; ?>
    ...
    <div class="dg-box-bd" style="padding:0;">
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;gap:0.5rem;padding:0.55rem 0.9rem;border-bottom:1px solid var(--border-color);<?= $it['is_active'] ? '' : 'opacity:0.5;' ?>flex-wrap:wrap;">
        <div style="display:flex;align-items:center;gap:0.5rem;min-width:0;">
          ...
          <div style="min-width:0;">
            ...
            <div style="color:var(--text-muted);font-size:0.72rem;direction:ltr;text-align:right;">
              <?= $it['url'] !== null ? h($it['url']) : '(خودکار در کدِ سایت)' ?>
            </div>
          </div>
        </div>
        <div style="display:flex;gap:0.25rem;align-items:center;flex-wrap:wrap;">
          ...
          <a href="?group=<?= h($group) ?>&delete=<?= $it['id'] ?>" class="btn btn-danger btn-sm" style="padding:0.15rem 0.4rem;font-size:0.7rem;" onclick="return confirm('این آیتمِ منو حذف شود؟');">حذف</a>
          <?php endif; ?>
        </div>
      </div>
