...
function saveBrandLogoUpload($editId) {
    ...
    /* هر فرمتی که آپلود شود (خواستهٔ کاربر: «خودت مرتبش کن») — به‌جز SVG
       (وکتور است، پردازش تصویر خام رویش معنا ندارد) — به یک PNG شفاف
       و trim‌شده تبدیل می‌شود؛ نمایش سایت فقط روی پس‌زمینهٔ شفاف درست کار
       می‌کند. اگر پردازش شکست بخورد (Imagick نبود، یا فایل غیرمنتظره)،
       خود فایل خام ذخیره می‌شود — بهتر از هیچ لوگو. */
    $finalExt = $ext;
    $saved = false;
    ...
    if ($saved) {
        /* tempnam()+rename() (مسیر نرمال‌سازی بالا) فایل را 0600 می‌سازد —
           فقط خود PHP می‌خواندش، nginx برای سرو کردن مستقیم فایل به آن
...
/* خواستهٔ کاربر: «به‌روزرسانی برند و مدلی که انجام میدم و ذخیره میکنم باز
   دسته‌بندی رو میبنده، اونو نبند تا خودم ببندمش» — بعد از ذخیره (بدون
   ریدایرکت، همین درخواست دوباره رندر می‌شود)، برند مربوط به همان ردیفی
   که ویرایش/اضافه شد باید باز بماند: اگر خود یک برند ذخیره شده همان
   برند، اگر یک مدل ذخیره شده برند مادرش. */
$savedBrandId = null;
...
/* حذف دسته‌جمعی همهٔ مدل‌های یک برند (خواستهٔ کاربر) — خود برند دست‌نخورده
   می‌ماند، فقط زیرمجموعه‌هایش پاک می‌شوند؛ ارتباطشان با محصولات هم پاک
   می‌شود (همان قاعدهٔ حذف تک‌مدل بالا، فقط یک‌جا برای همه). */
...
/* برندی که همین الان روی صفحه در حال ویرایش/افزودن است، باز بماند —
   از ذخیرهٔ همین درخواست ($savedBrandId)، یا از فرم باز (ویرایش یک
   مدل/برند، یا افزودن مدل تازه به یک برند). */
...
<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1rem;flex-wrap:wrap;gap:0.5rem;">
    ...
    <div style="display:flex;gap:0.5rem;flex-wrap:wrap;">
        <?php /* ۲۰۲۶-۰۹-۰۳: فرم از قبل «برند جدید» را پشتیبانی می‌کرد (برند
                مادر خالی = برند تازه) اما هیچ دکمه‌ای به آن اشاره نمی‌کرد —
                فقط با بازکردن categories.php بدون هیچ کوئری‌ای دیده می‌شد.
                همان «+ مدل» کنار هر برند، این‌جا برای خود برند. */ ?>
    ...
    <div class="dg-box">
        ...
        <div class="dg-box-bd" style="padding:0;">
            <?php /* ۲۰۲۶-۰۹-۰۳: پیش‌فرض همه بسته‌اند (خواستهٔ کاربر: «به‌صورت
                    ...
                    برندهای ≤۸ مدل خودکار باز بودند. تنها استثنا: برندی که
                    همین الان در حال ویرایش/افزودن است باز می‌ماند (خواستهٔ
                    بعدی کاربر: «ذخیره که می‌کنم دوباره می‌بندتش، نبند»). */ ?>
            ...
            <div class="pset-row">
                ...
                <div class="pset-sum-wrap">
                    ...
                    <?php /* خواستهٔ کاربر: «تمام زیرمجموعه‌های یک برند رو یک‌جا حذف کنم» —
                            حذف دسته‌جمعی همهٔ مدل‌ها، بدون لمس‌کردن خود برند. */ ?>
                    <?php if ($t['children']): ?>
                    <a href="?delete_all_models=<?= (int)$t['row']['id'] ?>" class="btn btn-danger btn-sm"
                       data-confirm="همهٔ <?= count($t['children']) ?> مدل «<?= h($t['row']['name']) ?>» حذف شوند؟ ارتباط آن‌ها با محصولات هم حذف می‌شود.">حذف همهٔ مدل‌ها</a>
                    <?php endif; ?>
