...
function cartClear() {
    $_SESSION['cart'] = [];
    /* روش ارسال انتخاب‌شده در سبد هم با سبد پاک می‌شود، وگرنه سفارش بعدی با
       روش سفارش قبلی شروع می‌شد. */
...
function cartAjaxState($productId = 0) {
    ...
    /* اگر بلوک ارسال روی صفحه نیست (shippingReady/AvailableMethods خاموش)،
       ردیف «مبلغ کل» سبد جداگانه و بدون کمک ship زنده می‌شود؛ همان‌جا هم
       مالیات باید لحاظ شود. وقتی بلوک ارسال هست، payable_html خود ship این
       کار را می‌کند (goodsTotal که پایین‌تر به آن داده می‌شود شامل مالیات است). */
...
/* ---------- وضعیت تازهٔ بلوک ارسال صفحهٔ سبد ----------
   با هر تغییر تعداد، وزن سبد و در نتیجه نرخ همهٔ روش‌ها و «کم‌ترین هزینه» عوض
   می‌شود (خواستهٔ مدیر: «وزن و هزینه بر اساس تعداد در سبد من تغییر کنه»). همان
   shippingCartSummary() سرور محاسبه می‌کند که رندر اول صفحه را هم ساخته، پس
   عدد زنده با عدد رندرشده و با عددی که در ثبت سفارش حساب می‌شود یکی است —
   هیچ نسخهٔ جاوااسکریپتی موازی از این محاسبه وجود ندارد.
...
function cartShipState(array $items, $goodsTotal) {
    ...
    foreach ($s['quotes'] as $q) {
        $rows[] = [
            ...
            /* روش محدود به شهر دیگر: با تغییر تعداد هم غیرفعال می‌ماند (شهر عوض
...
function handleCartAction() {
    ...
    /* دکمه‌های +/− سبد بدون جاوااسکریپت هم کار کنند: submit با name="step".
       در این حالت مقدار فیلد نادیده گرفته و روی تعداد فعلی سبد اعمال می‌شود. */
    $step = 0;
...
/* پاسخ مشترک همهٔ عملیات سبد: در حالت AJAX خروجی JSON و پایان کار، وگرنه
   همان آرایه تا هدر پیغام «توست» را نشان بدهد.
   حتی وقتی عملیات ناموفق بود (موجودی کافی نیست) وضعیت تازه برگردانده می‌شود
   تا مرورگر فیلد را به مقدار واقعی سبد برگرداند. */
