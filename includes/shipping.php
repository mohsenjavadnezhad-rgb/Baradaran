...
/* =====================================================================
   ...
   • فهرست روش‌ها در جدول `shipping_methods` است، پس مدیر می‌تواند روش
     ...
     ساخته نشده باشد (پیش از اجرای مهاجرت) همان هشت روشِ پیش‌فرضِ کد با
     همان کلیدهای تنظیماتِ قبلی کار می‌کنند تا صفحات خطا ندهند.
   • هزینهٔ ارسال به «مبلغ قابل پرداخت» اضافه می‌شود تا درگاه پرداخت
     مجموعِ کالا + ارسال را بگیرد.
   • «پس‌کرایه» (تیکِ freight_collect روی روش) یعنی محاسبه‌ای انجام نمی‌شود:
     هیچ رقمی به سفارش اضافه نمی‌شود و به مشتری فقط «پس‌کرایه» نشان داده
     می‌شود (خواستهٔ صریح مدیر — نه برآورد، نه رقمِ راهنما).
   • «نرخ‌نامه» (جدول `shipping_rates`): برای هر روش، برای هر شهر یک ردیفِ
     «هر چند کیلوگرم، چند تومان». هزینه = تعدادِ واحدهای وزنیِ سبد (رند به
     بالا) × قیمتِ هر واحد. مثال: تبریز، پست، هر ۱ کیلو ۵۰٬۰۰۰ تومان و سبدِ
   ...
   • «پیک مشهد» همیشه در فهرست می‌ماند و هیچ‌وقت پنهان نمی‌شود، ولی از
     ۱۴۰۵/۰۶/۰۳ اگر شهرِ پروفایلِ مشتری خوانده شود و مشهد نباشد، گزینه
     **غیرفعال** نشان داده می‌شود (خواستهٔ مدیر: «وقتی لوکیشنم غیر از مشهده باید
     ...
     shippingCityBlocked است و شهرِ محدودیت از نشانِ خودِ روش خوانده می‌شود.
     محافظ سرِ جایش است: شهرِ خالی یا ناشناخته چیزی را مسدود نمی‌کند، پس
     «مشهد مقدس»/«Mashhad»/غلط تایپی هیچ سفارشی را نمی‌بندد. نرخ‌نامه هم همین
     قاعده را دارد: نخواندنِ شهر یعنی «هزینه را نمی‌دانیم»، نه «ممنوع است».
   ===================================================================== */
require_once __DIR__ . '/db.php';

/* ---------- روش‌های پیش‌فرض (فقط تا وقتی جدول ساخته نشده) ---------- */
/* label: نام روش | icon: نام آیکون از iconLib() | hint: توضیحِ کمکی
   badge / badge_short : یادآوریِ محدودیت. badge_short برای کارت‌های فشردهٔ صفحهٔ
   ...
   مجاز نیست) — تصمیم مدیر، shippingAllowedPayKeys. نامِ ستون از نسخهٔ قبل
   مانده؛ معنایش امروز «پرداخت در محل مجاز است» است، نه «فقط پرداخت در محل».
   همین آرایه بذرِ اولیهٔ جدول `shipping_methods` هم هست (migrate-shiprates.php). */
...
/* آیا نرخ‌نامه «واحد وزن» دارد؟ (نرخِ به‌ازای هر چند کیلو، جای پله‌ایِ قبلی) */
...
/* ---------- فهرست شهرها ----------
   یک‌بار در هر درخواست خوانده و کش می‌شود؛ انتخابگرِ ادمین، انتخابگرِ شهر در
...
/* نام همهٔ شهرها (فهرست تخت) — شهرهای نرخ‌نامه هم اضافه می‌شوند تا اگر مدیر
...
/* نام رسمیِ شهر از روی چیزی که کاربر نوشته (برای یکدست‌سازیِ پروفایل و سفارش).
...
/* همان shippingCityGroups() ولی شهرهایی که فقط در نرخ‌نامه ثبت شده‌اند هم زیر
   «سایر» اضافه می‌شوند، تا شهرِ دستیِ مدیر از انتخابگرها جا نماند (همان کاری که
...
/* نام استان‌ها به ترتیبِ خودِ جدول شهرها (پایتختِ هر استان اول آمده است) */
function shippingProvinceNames() {
    return array_keys(shippingCityGroupsAll());
}

/* استانِ یک شهر، با همان مقایسهٔ ساده‌گیرِ shipNormCity. اگر شهر شناخته نشود ''
   برمی‌گردد تا فرم مقدارِ قبلیِ خودِ کاربر را دور نریزد. */
...
/* ---------- انتخابگرِ دوسطحیِ «استان ← شهر» برای فرم‌های مشتری ----------
   خواستهٔ کاربر: «اسم استان و شهر رو هم به صورت نوار کشویی بهم نشون بده».
   با انتخاب استان، فهرستِ شهر همان استان بازسازی می‌شود. دو نکتهٔ مهم:
   ۱) فهرستِ اولیه را *سرور* رندر می‌کند، پس بدون جاوااسکریپت هم درست است
      (وقتی استانی انتخاب نشده، همهٔ شهرها با optgroupِ استان دیده می‌شوند).
   ۲) اگر جدول شهرها ساخته نشده باشد، به همان دو فیلدِ متنیِ قبلی برمی‌گردد
      تا صفحه هرگز بی‌فیلد نماند.
   شهرِ ذخیره‌شده‌ای که در فهرست نیست، به‌عنوان گزینهٔ اول نگه داشته می‌شود تا
   ذخیرهٔ دوبارهٔ فرم آدرسِ مشتری را پاک نکند. */
function shippingProvinceCityFields($province, $city, $hint = '') {
    ...
    /* شهرِ شناخته‌شده حرفِ آخر را می‌زند: استان از خودِ شهر گرفته می‌شود تا
       فهرستِ فیلترشده همیشه شهرِ ذخیره‌شده را داشته باشد. */
    ...
    /* بازسازیِ فهرستِ شهر با تغییرِ استان. innerHTML بازسازی می‌شود (نه
       option.hidden) چون پنهان‌کردنِ گزینه در همهٔ مرورگرها یکسان نیست.
...
/* ---------- فهرست روش‌ها ----------
   ...
   و شکل هر عضو همان شکل قبلی است تا همهٔ مصرف‌کننده‌ها (صفحهٔ تسویه، پنل
   ادمین، سفارش‌های قدیمی) بدون تغییر کار کنند.
   شاملِ روش‌های حذف‌شده هم هست: سفارش‌های قدیمی باید نامِ روشِ خودشان را
...
/* تعریفِ یک روش — شاملِ روش‌های حذف‌شده، تا برچسبِ سفارش‌های قدیمی نشکند */
...
/* توضیح روش: متنِ مدیر (اگر نوشته شده باشد) وگرنه توضیحِ خودِ روش.
...
/* یادآوریِ محدودیت روش (مثل «فقط برای شهر مشهد») */
...
/* آیا این روش «پس‌کرایه» است؟ یعنی کرایه هنگام تحویل پرداخت می‌شود و به
   مبلغ سفارش اضافه نمی‌شود (تیکِ خودِ روش در پنل ادمین). */
function shippingIsFreightCollect($key) {
    $d = shippingMethodDef($key);
    return $d ? !empty($d['freight_collect']) : false;
}

/* هزینهٔ ثابتِ قدیمیِ روش. از نسخهٔ «نرخ بر پایهٔ شهر و وزن» به بعد در محاسبه
   استفاده نمی‌شود (تنها منبعِ هزینه، نرخ‌نامه است) و از پنل ادمین هم برداشته
...
/* متن هزینه برای نمایش — یک‌جا نگه داشته می‌شود تا همه‌جا یک‌شکل باشد.
   ...
   نشان داده می‌شود و هیچ رقمی — حتی رقمِ ثبت‌شده — بیرون نمی‌آید.
   قالبِ جمله‌ها از shippingRateTexts() می‌آید تا اسکریپتِ صفحهٔ تسویه هم
   بتواند دقیقاً همین متن را زنده بسازد. */
...
/* روش‌های فعالِ قابل نمایش در صفحهٔ تسویه.
   اگر مدیر همهٔ روش‌ها را خاموش کند آرایهٔ خالی برمی‌گردد و صفحهٔ تسویه
...
/* نام شهر را برای مقایسه یکدست می‌کند: نیم‌فاصله/کشیده حذف، ی و ک عربی به
   فارسی، ارقام فارسی به لاتین، فاصله‌های اضافه حذف. مقایسه «شاملِ هم بودن»
...
/* همهٔ ردیف‌های نرخ‌نامهٔ یک روش (مرتب بر اساس شهر).
...
/* واحد وزنِ یک ردیف نرخ‌نامه به کیلوگرم: «هزینه به‌ازای هر چند کیلو».
...
/* تعداد واحدهای وزنی، همیشه رند به بالا و حداقل یک واحد.
   ...
   ۰٫۰۰۰۰۱ خطای شناوری را می‌گیرد تا ۳ کیلوِ دقیق، ۴ واحد حساب نشود. */
...
/* ردیف‌های نرخ‌نامهٔ یک روش که با شهرِ نوشته‌شده می‌خوانند (فقط فعال‌ها).
...
function shippingRatesForCity($key, $city) {
    ...
    /* برابریِ کامل بر «شاملِ هم بودن» مقدم است: کسی که «قم» می‌نویسد نباید
       نرخِ «قمصر» را بگیرد وقتی خودِ «قم» ثبت شده. بعد از آن، کوچک‌ترین واحد
...
/* وزن سفارش از روی سبد خرید (کیلوگرم). وزن محصول اختیاری است، پس اگر هیچ
   کالایی وزن نداشته باشد صفر برمی‌گردد و نرخ‌نامه سراغِ «ردیف پایه» می‌رود.
...
/* ---------- محاسبهٔ نهایی هزینهٔ ارسال ----------
   ...
   1) شهر با ردیفی از نرخ‌نامه خواند ⇒ هزینه = تعداد واحدهای وزنی (رند به بالا)
      × قیمتِ هر واحدِ همان ردیف.  (source = rate)
...
function shippingResolveCost($key, $city = '', $weight = 0) {
    $res = [
        ...
        'has_rows' => false,  // این روش اصلاً نرخ‌نامه دارد؟
        'weight'   => (float)$weight,
        'unit'     => 0.0,
        'units'    => 0,
    ...
    /* «نرخ‌نامه دارد» فقط ردیف‌های فعالِ شهردار را می‌شمارد — دقیقاً همان
...
/* ---------- متن‌های نرخ‌نامه ----------
   در یک‌جا نگه داشته می‌شوند تا رندرِ سرور (که بدون جاوااسکریپت هم باید درست
   باشد) و اسکریپتِ زندهٔ صفحهٔ تسویه دقیقاً یک جمله بگویند. */
...
/* جملهٔ توضیحیِ زیر انتخابگر ارسال برای روشِ انتخاب‌شده */
...
/* آیا این روش «باربری» است؟ بخش «هزینه‌های باربری» تنظیمات فقط زیر همین روش
   به مشتری نشان داده می‌شود. کلیدِ پیش‌فرض `barbari` است، ولی مدیر می‌تواند
   روش باربریِ دیگری هم بسازد، پس از نام روش هم شناسایی می‌شود. */
...
/* ---------- دادهٔ محاسبهٔ زندهٔ صفحهٔ تسویه ----------
   برای هر روش: پس‌کرایه بودن (c)، باربری بودن (b) و ردیف‌های فعالِ نرخ‌نامه‌اش
   (r: شهرِ یکدست‌شده، نام شهر، واحد وزن، قیمتِ هر واحد). اسکریپت با همین داده
...
/* تصمیم مدیر: «پرداخت در محل» فقط برای ارسال با پیک معنا دارد، ولی روی پیک
   پرداخت اینترنتی هم باید باز باشد (ممکن است مشتریِ مشهدی آنلاین پرداخت کند).
   پس:
   • روش ارسالِ cod_only (پیک) ← همهٔ روش‌های پرداخت، از جمله «پرداخت در محل»
   • هر روش ارسال دیگر         ← هر روشی به‌جز «پرداخت در محل»
   نامِ ستون cod_only از نسخهٔ قبل مانده؛ معنایش «پرداخت در محل مجاز است». */
function shippingCodAllowed($key) {
    $d = shippingMethodDef($key);
    return $d ? !empty($d['cod_only']) : false;
}

/* نامِ قبلی، برای سازگاری با فراخوان‌های موجود */
function shippingIsCodOnly($key) {
    return shippingCodAllowed($key);
}

/* روش‌های پرداختِ مجاز برای یک روش ارسال.
   تا وقتی مشتری روش ارسالی انتخاب نکرده، همه مجاز است.
   نکتهٔ مهم: اگر این قاعده هیچ روش پرداختی باقی نگذارد (مثلاً مدیر فقط
...
/* پیام راهنمای همین قاعده. متن در یک‌جا نگه داشته می‌شود تا سرور (اعتبارسنجی و
   رندر اولیه) و اسکریپت صفحهٔ تسویه (به‌روزرسانی زنده) دقیقاً یک جمله بگویند. */
...
/* توضیحِ «پس‌کرایه» در صفحهٔ ثبت سفارش.
   جای پیامِ قاعدهٔ پرداخت را گرفته است — تصمیم مدیر: «این مال اینه که مقصد مشهد
   ...
   دریافت میشود». پس آن پیام دیگر رندر نمی‌شود (خودِ گزینهٔ کم‌رنگ و غیرفعال
   گویاست) و تنها همین جمله می‌آید، آن هم فقط روی روش‌های پس‌کرایه.
   shippingPayRuleNote بالا حذف نشده چون در اعتبارسنجیِ POST — جایی که واقعاً
...
/* ---------- برآورد ارسال برای صفحهٔ سبد خرید ----------
   سبد خرید هنوز روش ارسال ندارد، پس هزینهٔ همهٔ روش‌های فعال با شهرِ پروفایلِ
   مشتری و وزن سبد حساب و کنار هم نشان داده می‌شود («هم در سبد خرید و هم در
   تسویه» — تصمیم مدیر). محاسبه دقیقاً همان shippingResolveCost() است، پس
   رقمِ سبد با رقمِ تسویه یکی است. */
function shippingCartQuotes($city, $weight) {
    ...
    foreach (shippingAvailableMethods() as $k => $d) {
        ...
        /* روشِ مسدود هیچ رقمی نمی‌گیرد: نه نرخی نشان داده می‌شود، نه در
           «کم‌ترین هزینه» می‌آید، نه به مبلغ سفارش راه دارد. جای رقم را همان
           نشانِ محدودیت («مشهد») می‌گیرد (shippingQuoteBadgeOnly) و خودِ ردیف
...
/* کم‌ترین هزینهٔ ارسالِ معلوم (برای خط «از … تومان» در سبد). اگر هیچ روشی نرخ
   نداشته باشد null برمی‌گردد. */
function shippingCheapestQuote(array $quotes) {
    $best = null;
    foreach ($quotes as $q) {
        if (!empty($q['blocked'])) continue;   // روشِ غیرفعال نمی‌تواند «کم‌ترین هزینه» باشد
...
/* ---------- روشی که به یک شهر محدود است («ارسال با پیک (مشهد)») ----------
   ...
   شهرِ محدودیت از خودِ «نشانِ» روش خوانده می‌شود (badge_short و اگر نبود badge):
   ستون جداگانه‌ای برای شهر روی روش‌ها وجود ندارد و همین نشان تنها جایی است که
   مدیر محدودیت را می‌نویسد؛ پس روشِ پیکِ شهرِ دیگری هم که مدیر بسازد خودکار
   همین قاعده را می‌گیرد (مثل shippingIsBarbari که از نام روش تشخیص می‌دهد).
   واژه‌های عمومیِ نشان («فقط برای شهر …») کنار گذاشته می‌شوند و باقی‌مانده تنها
   وقتی «شهر» شمرده می‌شود که در فهرست شهرها باشد — پس نشانی مثل «ارسال سریع»
   هیچ روشی را محدود نمی‌کند.
   دو محافظ عمداً باقی مانده‌اند تا این قاعده هیچ سفارشی را قفل نکند:
   • شهرِ خالی یا ناشناخته هرگز مسدود نمی‌کند؛ نخواندنِ شهر یعنی «نمی‌دانیم»،
     نه «ممنوع» (همان قاعدهٔ نرخ‌نامه).
   • اگر تنها یک روش ارسال فعال باشد مسدود نمی‌شود، وگرنه سفارش‌گیری می‌خوابید
     (همان منطقِ «هیچ‌وقت قفل نکن» در shippingAllowedPayKeys). */
function shippingMethodCityLimit($key) {
    ...
    if ($t !== '') {
        ...
        foreach (shippingCityNames() as $cn) {
            $n = shipNormCity($cn);
            if ($n === '') continue;
            if ($n === $t) { $best = (string)$cn; break; }   // نشان دقیقاً نامِ شهر است («مشهد»)
            /* نشانِ بلند («فقط برای شهر مشهد») نامِ شهر را در خود دارد. مقایسه با
               فاصله در دو طرف انجام می‌شود تا نیم‌کلمه شهر شمرده نشود («شهری»
               نباید «ری» بخواند و «رشتخوار» نباید «رشت» بخواند)؛ بلندترین نامِ
               خواننده برنده است تا شهرِ چندکلمه‌ای هم درست دربیاید. */
...
/* آزمونِ عضویت در فهرست شهرها: نامِ رسمیِ شهر برمی‌گردد و اگر ناشناس بود ''.
   برخلاف shippingCityCanonical() که ورودیِ ناشناس را عیناً پس می‌دهد (تا نوشتهٔ
   مشتری دور نریزد)، این یکی برای «آیا این تکه، نامِ شهر است؟» ساخته شده.
   نگاشتِ یکدست‌شده یک‌بار در هر درخواست ساخته می‌شود. */
...
/* آیا این روش، پیکِ درون‌شهری است (نه پست/باربری)؟ سه نشانه، به همان ترتیبِ
   اعتماد: نشانِ شهریِ خودِ روش («مشهد») که shippingMethodCityLimit() درمی‌آورد،
   بعد کلید، بعد برچسب — دقیقاً همان اصطلاحِ shippingIsBarbari()، تا اگر مدیر
   پیکِ شهر دیگری هم اضافه کرد بی‌نیاز از تغییرِ کد شناخته شود. */
...
/* آیا نشانیِ این شهر در محدودهٔ پیکِ این روش است؟ همان مقایسهٔ دوطرفهٔ
   shippingCityBlocked() ولی بدون گاردِ «کمتر از دو روش» — اینجا حرفی از
   مسدودکردنِ سفارش نیست، فقط تشخیصِ «درون‌شهری بودنِ مقصد» است. */
...
/* شهرِ مبنای این قاعده: شهرِ پروفایلِ مشتریِ واردشده. یک منبع تا صفحهٔ سبد،
   پاسخ AJAX و صفحهٔ تسویه هر سه یک تصمیم بگیرند. مهمان یا شهرِ ثبت‌نشده ⇒ ''
...
/* توضیحِ زیر فهرستِ سبد برای روش‌های مسدود — یک‌جا نگه داشته می‌شود تا رندرِ
   سرور و هر مصرف‌کنندهٔ دیگری یک جمله بگویند. خودِ ردیف کم‌رنگ و غیرفعال است؛
...
/* ---------- روش ارسالِ انتخاب‌شده، در نشست ----------
   ...
   مقدارِ نشست هر بار اعتبارسنجی می‌شود تا روشی که مدیر بعداً خاموش یا حذف
   کرده به سفارش نرسد. */
function shippingSessionMethod() {
    ...
    /* شهرِ پروفایل بعد از انتخاب عوض شده و این روش دیگر مجاز نیست (پیکِ مشهد
       برای مشتریِ تهرانی). با برگشتنِ '' صفحهٔ سبد دوباره «انتخاب کنید» می‌گوید
       و صفحهٔ تسویه به سبد برمی‌گرداند، تا سفارش با روشِ نامعتبر ثبت نشود. */
...
function shippingSetSessionMethod($key) {
    $key = trim((string)$key);
    $ok  = shippingAvailableMethods();
    /* بررسیِ سمت سرور: رادیوی روشِ مسدود در فرم disabled است، ولی بدون
       جاوااسکریپت یا با فرمِ دست‌کاری‌شده هم نباید در نشست بنشیند. */
...
/* ---------- خلاصهٔ ارسالِ صفحهٔ سبد خرید ----------
   یک‌جا ساخته می‌شود تا سه مصرف‌کننده‌اش هیچ‌وقت با هم اختلاف نداشته باشند:
   رندرِ اولِ cart.php، پاسخِ AJAX هنگام تغییر تعداد، و بازبینیِ صفحهٔ تسویه.
   ...
   هر بار از نو حساب می‌شود. شهرِ خالی هم پذیرفته است: فهرست روش‌ها می‌آید
...
/* آیا این ردیفِ برآورد باید فقط نشانِ «فقط مشهد» را نشان بدهد؟
   خواستهٔ مدیر: «اون قسمت ارسال با پیک (مشهد) که نوشته پس از هماهنگی اونو
   بردار و فقط بنویس فقط مشهد». پس وقتی روشی نرخِ معلومی ندارد ولی نشانِ
   محدودیت دارد، همان نشان جای رقم را می‌گیرد. روش‌های «پس‌کرایه» از این قاعده
   بیرون‌اند، چون خودِ کلمهٔ «پس‌کرایه» اطلاع مفیدی است. */
