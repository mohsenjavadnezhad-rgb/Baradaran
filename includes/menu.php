<?php
/* =====================================================================
   منوهای سایت (نوار بالای صفحه + «دسترسی سریع»ِ فوتر) — قابل مدیریت از پنل
   ادمین (admin/menus.php): افزودن، ویرایشِ نام/آیکون، فعال/غیرفعال کردن،
   حذف و جابه‌جاییِ ترتیب.
   ---------------------------------------------------------------------
   هر آیتم یا «آزاد» است (item_key خالی: لینکِ ساده با برچسب/آدرس/آیکونِ
   دلخواه — کاملاً حذف‌شدنی) یا «سیستمی» (item_key پر: مثل مگامنوی فروشگاه
   یا لینکِ حساب‌کاربری/ورود که آدرس یا محتوایش وابسته به منطقِ صفحه است و
   در includes/header.php و includes/footer.php ساخته می‌شود، نه از ستونِ
   url؛ فقط برچسب/آیکون/ترتیب/فعال‌بودنش از دیتابیس می‌آید و حذفِ واقعی
...
/* چیدمانِ پیش‌فرضِ اولیهٔ سایت — هم برای بذرِ migrate-menus.php و هم برای
...
/* آیتم‌های فعالِ یک منو، به ترتیبِ نمایش — برای رندرِ سمتِ سایت.
   قبل از مهاجرت (یا در صورت خطای غیرمنتظره) به چیدمانِ پیش‌فرض برمی‌گردد تا
...
/* همهٔ آیتم‌ها (غیرفعال‌ها هم) — برای فهرستِ پنلِ مدیریت */
...
/* عددِ ترتیبِ بعدی برای آیتمِ تازه (همیشه ته لیست) */
...
function menuGroupLabel($group) {
    return $group === 'footer' ? 'دسترسیِ سریعِ فوتر' : 'منوی بالای سایت';
}

/* آیکون‌های پیشنهادی برای انتخابگرِ آیکون در پنلِ مدیریت — زیرمجموعه‌ای از
   includes/icons.php که برای لینکِ منو معنادارند. */
