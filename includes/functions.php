...
/* آیا پرداخت این سفارش باز است؟ پیش از مهاجرت (یا وقتی روند ارسال اصلا
   ...
   ۲۰۲۶-۰۹-۰۳ (خواستهٔ کاربر): «وقتی تیک بررسی موجودی رو نزدم دیگه نیازی
   نیست تو order-success.php باز بذاریش» — این گیت پیش از این همیشه فعال
   بود، حتی وقتی stockGateActive() (تنظیمات ← تأیید موجودی) اصلا خاموش
   است و ادمین اساسا قصد ندارد پیش از پرداخت موجودی را تأیید کند؛ نتیجه‌اش
   این بود که مشتری بی‌جهت پشت کادر «در انتظار بررسی موجودی» می‌ماند و
   کسی هم قرار نبود آن تیک را بزند. با خاموش‌بودن آن کلید، این گیت هم
   بی‌درنگ باز است — track_stock_at همچنان NULL می‌ماند (پس نه کادر زرد
   انتظار نه کادر سبز تأیید نشان داده می‌شود، اصلا این بخش از صفحه دیده
   نمی‌شود) ولی خود مرحلهٔ «بررسی موجودی» در چک‌لیست پیگیری ادمین
   (order-detail.php) دست‌نخورده می‌ماند تا اگر خواست همچنان تیک بزند. */
function orderPaymentUnlocked($order) {
    if (!trackStockReady()) return true;
    if (!stockGateActive()) return true;
