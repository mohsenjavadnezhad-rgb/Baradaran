...
/* حالتِ نمایشِ نامِ انگلیسیِ برند (اسلاگ) روی تگ‌های برندِ shop.php:
   'off' (پیش‌فرض، هیچ‌وقت نشان نده) / 'hover' (فقط با نگه‌داشتنِ موس) /
   'always' (همیشه به‌جای فارسی). از admin/categories.php تنظیم می‌شود —
   [[batch9-fixes]]. مقدارِ نامعتبر یا خالی → 'off'، نه خطا. */
...
/* بخش‌های صفحهٔ تنظیمات ادمین — منبعِ یگانه برای نوار کشویی سایدبار
   (admin/layout-top.php) و خودِ صفحه (admin/settings.php?sec=...).
   هر بخش جدا نمایش و جدا ذخیره می‌شود، پس کلید بخش هم در آدرس و هم در فیلد
   مخفیِ فرم می‌آید تا هنگام ذخیره فقط کلیدهای همان بخش نوشته شوند. */
function settingsSections() {
    return [
        ...
        /* پرداخت اعتباری/اقساطی از «درگاه پرداخت» جدا شد چون سرویس‌دهندهٔ
           متفاوتی دارد (اعتبارسنجیِ BNPL، نه دروازهٔ بانکی) و با هم بخشِ
        ...
        /* نرخ‌نامه صفحهٔ خودش را دارد چون با هر شهرِ تازه یک ردیف اضافه می‌شود و
...
/* چند شماره در یک textarea (هر شماره یک خط، یا جداشده با کاما) → آرایهٔ تمیز.
   اگر کلید جمع خالی بود، به مقدار تکیِ قدیمی برمی‌گردد. */
...
/* آیا حساب، همکارِ تأییدشده است؟ (فقط برای نشان «تأییدشده» — روی قیمت اثری ندارد) */
function isApprovedPartner($c) {
    return $c && ($c['customer_type'] ?? '') === 'partner' && ($c['partner_status'] ?? '') === 'approved';
}

/* ---------- سبد خرید و مالکیتِ آن ----------
   سبد خرید در نشست ذخیره می‌شود (`$_SESSION['cart']`) و نشست به مرورگر بسته است،
   نه به حساب کاربری. پس اگر کسی خارج شود و با حسابِ دیگری وارد شود، سبدِ حسابِ
   ...
   نشون میده».
   راه‌حل: کنارِ خودِ سبد، شناسهٔ صاحبش هم نگه داشته می‌شود (`cart_owner`؛ ۰ یا
   نبودنش یعنی «سبدِ مهمان»). قاعده:
   • سبدِ مهمان با ورود به همان حساب واگذار می‌شود — کسی که پیش از ورود سبد را پر
     کرده و سرِ تسویه وارد می‌شود نباید سبدش را از دست بدهد.
   • سبدِ حسابِ دیگر هرگز به حسابِ تازه نمی‌رسد؛ دور ریخته می‌شود.
   • خروج، سبد را هم می‌برد، وگرنه نفرِ بعدی روی همین مرورگر آن را می‌دید.
   روش ارسالِ انتخاب‌شده هم با سبد می‌رود (همان کاری که cartClear() می‌کند)،
   وگرنه سبدِ خالیِ حسابِ تازه با روشِ ارسالِ حسابِ قبلی شروع می‌شد. */
function cartForgetSession() {
    unset($_SESSION['cart'], $_SESSION['cart_owner'], $_SESSION['ship_method']);
}

/* نگهبانِ مالکیت، یک‌بار در هر درخواست (انتهای همین فایل صدا زده می‌شود).
   لایهٔ دومِ همان قاعده است تا مسیرهای دیگر هم پوشیده شوند: نشستی که پیش از این
...
function cartOwnerSync() {
    ...
    if ($owner !== 0) cartForgetSession();             // سبدِ حسابِ دیگر ⇒ دور ریخته می‌شود
    if ($now !== 0) $_SESSION['cart_owner'] = $now;    // سبدِ مهمان ⇒ واگذاری به همین حساب
}

function loginCustomer($customer) {
    global $pdo;
    $id = (int)$customer['id'];
    /* تصمیم درست‌ترین جا همین‌جاست: لحظهٔ عوض‌شدنِ حساب. (cartOwnerSync() در
       ابتدای درخواست با شناسهٔ حسابِ قبلی اجرا شده و این تغییر را ندیده.) */
    $owner = isset($_SESSION['cart_owner']) ? (int)$_SESSION['cart_owner'] : 0;
...
/* آیا ورود مشتری به کد تأیید پیامکی نیاز دارد؟
   ...
   نکتهٔ مهم: پیش‌فرضِ نبودِ کلید در جدول settings عمداً «لازم است» گذاشته شده
   تا روی نصب‌های قدیمی‌تر ورودِ بی‌تأیید به‌طور ناخواسته باز نشود. */
...
function smsSendOtp($mobile, $code) {
    ...
    // موفقیت = کد HTTP 2xx و (نبودِ فیلد status یا status==1 در پاسخ SMS.ir)
    $apiOk = !empty($res['ok']);
...
/* ---------- اطلاع‌رسانی پیامکیِ مراحل روند ارسال ----------
...
/* نقشهٔ «ستونِ مرحله → کلیدِ تنظیمات + متنِ پیش‌فرض».
   منبعِ یگانه برای صفحهٔ تنظیمات و برای فرستنده، تا هر دو یک متن را بشناسند. */
function orderTrackSmsKeys() {
    return [
        'track_stock_at'           => ['key' => 'sms_track_stock',      'default' => 'موجودیِ سفارش {order} بررسی و تأیید شد؛ اکنون می‌توانید سفارش را پرداخت کنید. {site}'],
...
/* متنِ خامِ پیامکِ یک مرحله (بدون جایگزینیِ متغیرها). '' یعنی این مرحله پیامک ندارد.
   getSettingRaw (نه getSetting) تا اگر مدیر عمداً متن را خالی کند، همان «خاموش»
...
/* ارسال یک پیامکِ متنِ آزاد. همیشه از سرویس bulk سرویس‌دهنده استفاده می‌شود
   چون «verify» فقط قالبِ ثابت می‌فرستد و متنِ مرحله متغیر است.
   در حالت آزمایشی یا پیکربندیِ ناقص، فقط در storage/sms.log ثبت می‌شود. */
...
/* پیامکِ یک مرحله برای مشتریِ همان سفارش.
   $order باید ردیفِ به‌روزشدهٔ سفارش باشد (تا {code} و {courier} تازه باشند).
   خروجی: [ok, test, error, skipped] — هیچ خطایی به بالا پرت نمی‌شود چون
   ناموفق‌بودنِ پیامک نباید ثبتِ مرحله را خراب کند. */
...
/* مرحلهٔ «تحویل به پست» + کد رهگیری (migrate-tracking2.php).
   ...
   بقیهٔ روند ارسال سرِ جایش بماند و فقط این یک مرحله نشان داده نشود
...
/* آیا پرداختِ این سفارش باز است؟ پیش از مهاجرت (یا وقتی روندِ ارسال اصلاً
   فعال نیست) گیت خاموش می‌ماند — همان رفتارِ قبلی (پرداختِ بی‌درنگ) — تا
   نصب‌های قدیمی‌تر نشکنند. بعد از مهاجرت: تا مدیر «بررسی موجودی» را تیک
   نزند (یا این مرحله خودکار طی نشده باشد)، هیچ روشِ پرداختی — حتی درگاهِ
...
/* اگر «بررسی عکس نمونهٔ قطعه» همین سفارش را از قبل approved و stock_ok=1
   کرده، گیتِ «بررسی موجودی» خودکار طی می‌شود — کاری که مدیر همان‌جا انجام
...
/* آیا نشانِ «فروخته شد» روی جدول آفرها ساخته شده؟ */
...
/* ===================== تأیید عکس نمونهٔ قطعه =====================
   مرحله‌ای اختیاری میان سبد خرید و ثبت سفارش: مشتری چند عکس از زوایای مختلفِ
   قطعهٔ موردنیازش می‌فرستد، مدیر با محصولِ سبد مقایسه می‌کند و در همان صفحه
   موجودی را هم تأیید می‌کند؛ سپس مشتری «مرحلهٔ بعد» را می‌زند و به ثبت سفارش
   و پرداخت می‌رود. چون مرجوعی ممکن نیست، این مرحله جلوی خریدِ قطعهٔ اشتباه را
   می‌گیرد. مشتری می‌تواند این مرحله را رد کند (کلیدِ «رد کردن»).
   کلِ مرحله با کلیدِ partcheck_enabled در تنظیمات خاموش می‌شود. */
...
/* امضای کالاهای سبد (فقط شناسهٔ محصول‌ها، نه تعداد) — کم/زیادکردنِ تعداد
   تأییدِ گرفته‌شده را باطل نمی‌کند، ولی افزودنِ قطعهٔ تازه بله. */
...
/* آخرین درخواستِ بازِ (بی‌سفارشِ) این مشتری */
...
/* درخواستِ گره‌خورده به یک سفارش (برای صفحهٔ جزئیات سفارشِ ادمین) */
...
/* تعداد درخواست‌های در انتظار بررسی — نشانِ کنار منوی ادمین */
...
/* آپلودِ چند عکسِ مشتری → آرایهٔ نام فایل‌های ذخیره‌شده.
   همان اعتبارسنجیِ گالریِ محصول: پسوندِ مجاز + سقفِ حجم. نامِ فایلِ کاربر هرگز
   استفاده نمی‌شود (نامِ تازه ساخته می‌شود) تا مسیرپیمایی و پسوندِ دوگانه ممکن
   نباشد. */
function partCheckSaveUploads($field, $max = 8) {
    ...
    for ($i = 0; $i < $count && count($saved) < $max; $i++) {
        ...
        if (!@getimagesize($_FILES[$field]['tmp_name'][$i])) continue;   /* واقعاً عکس باشد */
...
/* متنِ درشتِ بالای صفحه — مدیر می‌تواند در تنظیمات جای آن متنِ خودش را بگذارد */
function partCheckNotice() {
    ...
    return 'پیش از ثبت سفارش، عکس قطعهٔ خودتان را برای ما بفرستید تا کارشناسان ما آن را با '
         ...
         . 'دقیقاً همان کالایی باشد که نیاز دارید و روی خودروی شما بخورد — چون پس از خرید، '
         . 'امکان مرجوعی و تعویض قطعه وجود ندارد.';
}

/* ===================== روند پیگیری سفارش =====================
   لایه‌ای در کنارِ ENUM موجودِ orders.status (نه جایگزین آن)؛ هر مرحله یک ستون
   DATETIME است که ادمین با تیک‌زدن مهر زمان می‌کند. ترتیب آرایه = ترتیب نمایش.
   «تحویل به پست» فقط وقتی ستونش ساخته شده باشد وارد فهرست می‌شود؛ کنارِ
   «تحویل به پیک» می‌نشیند تا مدیر هرکدام را که واقعاً انجام شده تیک بزند.
...
function orderTrackSteps() {
    ...
    /* «بررسی موجودی» همیشه اولین مرحله است (خواستهٔ مدیر) — تا این مرحله ثبت
       ...
       همین فایل). اگر سفارش قبلاً از «بررسی عکس نمونهٔ قطعه» با موجودیِ
...
/* آدرس رهگیری مرسولهٔ پست برای کدی که ادمین وارد کرده.
   فقط کدِ رقمی لینک می‌شود تا ورودیِ بی‌ربط، لینکِ خراب نسازد. */
...
/* ---------- شهرِ مقصدِ سفارش ----------
   جدول orders ستونِ شهر ندارد؛ checkout.php نشانی را به شکل
   «استان - شهر - نشانی - کدپستی» در customer_address می‌چیند و استان می‌تواند
   خالی باشد، پس فقط دو بخشِ نخست نامزدِ نامِ شهرند. همین محدودکردن است که
   خیابانی به‌نامِ یک شهر («بلوار تهران») را شهرِ مقصد نمی‌خواند.
...
/* ---------- شیوهٔ تحویلِ سفارش ----------
   تا روند ارسالِ مشتری فقط مرحله‌های مربوط به خودِ سفارش را نشان دهد:
     'courier' = پیکِ درون‌شهری ⇒ «تحویل به پست» برای این سفارش بی‌معناست
     'ship'    = پست/باربری/…   ⇒ «در حال جستجوی پیک» و «تحویل به پیک» بی‌معناست
     ''        = نمی‌دانیم      ⇒ هیچ مرحله‌ای پنهان نمی‌شود
   روشِ ارسالِ خودِ سفارش حرفِ آخر است، چون همان چیزی است که مشتری خریده و
   درست‌تر از حدس‌زدن از شهر است (مشتریِ مشهدی هم می‌تواند پستِ پیشتاز بخرد و
   آن‌وقت کد رهگیری‌اش را لازم دارد). فقط برای سفارش‌های قدیمیِ بدون روشِ ارسال،
   شهرِ نشانی جای آن را می‌گیرد. */
...
/* همان orderTrackSteps() ولی فقط مرحله‌هایی که مشتریِ این سفارش باید ببیند.
   خودِ orderTrackSteps() دست‌نخورده می‌ماند: تیک‌های پنل ادمین و کارتِ تنظیماتِ
   پیامک به همهٔ مرحله‌ها نیاز دارند («فقط سمت ادمین باشه»)؛ این فیلتر تنها
   برای چشمِ مشتری است. سه مرحلهٔ تأیید/جمع‌آوری/ارسال هرگز حذف نمی‌شوند. */
...
function renderOrderTimeline($order, $showEmptyNote = true) {
    ...
    foreach ($steps as $col => $def) {
        ...
        /* کد رهگیری پست: قابل انتخاب برای کپی، و اگر رقمی بود لینک به رهگیریِ پست */
...
/* ===================== کارتِ سفارش در پنل مشتری =====================
   یک رندرکنندهٔ مشترک برای سه جا: فهرستِ «سفارش‌های در جریان» در account.php،
   صفحهٔ «سفارش‌های گذشته» و پاسخِ بروزرسانیِ درجا (frag=orders). پیش‌تر همین
   نشانه‌گذاری داخل account.php بود؛ یک‌جا نگه‌داشتنش جلوی واگرایی سه نسخه را
   می‌گیرد. */

/* شمارهٔ نمایشیِ سفارش — تاریخِ ثبت (میلادی، هم‌الگوی شمارهٔ فاکتورِ قبلی در
   admin/invoice.php) + شناسهٔ یکتا با صفرهای ابتدایی. خودِ id همچنان کلید
   اصلی و مبنای URLها می‌ماند (order-detail.php?id=، order-success.php?id=،
   کلیدهای خارجیِ order_items/payments/...)؛ این فقط شکلِ نمایشی است که
   خواستهٔ کاربر بود: «بر اساس سال ماه روز و شماره یونیک». همه‌جا که قبلاً
...
/* برچسب‌های فارسیِ ENUM ستون orders.status */
...
function renderCustomerOrderCard($order) {
    ...
    if ($payOn) {
        ...
        if ($ps !== 'paid' && paymentIsOnline($pm) && $st !== 'cancelled' && orderPaymentUnlocked($order)) {
            ...
            /* آنلاین است ولی گیتِ «بررسی موجودی» هنوز باز نشده */
            $out .= '<div class="account-order-row" style="justify-content:flex-end;">'
...
/* ناحیهٔ زندهٔ فهرست سفارش‌ها: هم در نخستین رندرِ صفحه و هم در پاسخِ
   بروزرسانیِ درجا از همین تابع ساخته می‌شود، پس دو نسخه از یک HTML نداریم. */
...
/* ===================== بنرهای کناری صفحهٔ اصلی =====================
   بنر راست («تخفیف ویژه») زنده از خودِ محصولِ تیک‌خورده ساخته می‌شود — هیچ ردیف
   بنری ذخیره نمی‌شود تا همیشه با محصول هم‌گام بماند.
   بنر چپ («آفر زمان‌دار») توسط ادمین در جدول timed_offers ساخته می‌شود. */

/* جدیدترین محصولِ «فروش ویژه» (تیکِ is_special در پنل ادمین) */
...
/* همهٔ آفرهای زمان‌دارِ فعال — ورودیِ اسلایدرِ صفحهٔ اصلی.
   آفرِ منقضی حذف نمی‌شود: بنرش می‌ماند و روی آن «مهلت خرید تمام شد» نوشته
   می‌شود (خواستهٔ کاربر)؛ فقط به انتهای صف می‌رود تا آفرهای زندهٔ قابلِ خرید
   اول دیده شوند. «فروخته شد» هم همین‌طور — بنر با مهرِ گوشه می‌ماند.
   فقط تیکِ «نمایش/مخفی» ادمین (is_active) بنر را برمی‌دارد.
...
/* تکِ اولین آفرِ فعال (نگه‌داشته شده برای فراخوان‌های قدیمی) */
function getActiveTimedOffer() {
    $rows = getActiveTimedOffers(1);
    return $rows ? $rows[0] : null;
}

/* هر چند ثانیه اسلایدِ آفرها عوض شود؟ ادمین در «مدیریت بنرها» تعیین می‌کند.
...
/* جمعِ مالیاتِ سبدِ فعلی — جدا از getCartTotal() نگه داشته شده چون خیلی جا
   (نرخِ ارسال، حدِ پایینِ پرداخت اعتباری، …) از getCartTotal() به‌عنوانِ
   «جمعِ کالاها»ی خالص استفاده می‌کنند و نباید بی‌سروصدا معنایش عوض شود. */
...
/* شمارهٔ فنی خودکار: «اختصارِ قطعه - اختصارِ مدل - شمارهٔ یونیک»
...
/* میانگین و تعدادِ نظرهای تأییدشدهٔ همهٔ محصولات — یک کوئری برای کل درخواست.
...
/* [میانگین, تعداد] برای یک محصول — بدون نظرِ تأییدشده: [0, 0] */
function getProductRating($productId) {
    $all = ratingsAll();
    return $all[(int)$productId] ?? [0.0, 0];
}

/* پنج ستاره در دو لایه: پایهٔ خاکستری + لایهٔ رنگیِ بریده‌شده به درصدِ میانگین.
   با این روش «نیم‌ستاره» دقیق (مثلاً ۴.۳ → ۸۶٪) نمایش داده می‌شود و به آیکن
...
/* نسخهٔ فشرده برای کارت‌های فهرست — محصولِ بی‌نظر چیزی نشان نمی‌دهد */
...
/* پرسش‌های تأییدشدهٔ یک محصول، هر پرسش با کلید 'answers'.
   پاسخِ تأییدشده‌ای که پرسشش تأیید نشده باشد نمایش داده نمی‌شود (درست است). */
...
/* نام نمایشیِ نویسندهٔ نظر/پرسش/پاسخ */
...
/* نگهبانِ مالکیتِ سبد، یک‌بار در هر درخواست. همین‌جا صدا زده می‌شود چون تنها
