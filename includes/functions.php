...
function settingsSections() {
    return [
        ...
        /* ۲۰۲۶-۰۹-۰۳: از داخل بخش «سال تولید خودرو» بیرون آمد و دسترسی
           مجزای خودش را گرفت (خواستهٔ کاربر) — قبلاً هر دو کلید همان‌جا
           بودند، اما موضوعاً جداست (مراحل ناوبری، نه بازهٔ سال). */
...
/* ۲۰۲۶-۰۹-۰۳: لوگوی خام آپلودی را به یک PNG شفاف تمیز تبدیل می‌کند —
   ...
   عمداً همیشه بدون گارد «آیا لازم است» اجرا می‌شود — خواندن آلفای هر
   پیکسل از طریق Imagick برای PNG‌های ایندکس‌شده/بدون‌کانال‌آلفای
   واقعی قابل‌اعتماد نبود (چند لوگوی واقعاً توپر اشتباهی «شفاف» تشخیص
...
/* ۲۰۲۶-۰۹-۰۳: آیا ستون‌های «بدون نیاز به برند/مدل/سال خودرو» روی products
   ...
   محصولات (مثل لوازم عمومی) اصلاً به یک خودروی خاص مقید نیستند — با
...
function getProducts($filters = []) {
    ...
    /* ۲۰۲۶-۰۹-۰۳: محصول «بدون نیاز به برند/مدل» (تیک فرم محصول) باید در
       هر فیلتر برند/مدلی دیده شود، حتی اگر اصلاً به هیچ دسته‌ای وصل نباشد
