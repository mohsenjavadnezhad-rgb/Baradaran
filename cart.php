...
/* ---------- انتخاب روش ارسال و برآورد هزینه، همین‌جا در سبد ----------
   ...
   بنویسد نرخِ تبریز را همین‌جا می‌بیند — و وزن از مجموع وزنِ کالاهای سبد ضرب در
   تعداد، پس با هر کم/زیادکردن تعداد عوض می‌شود. محاسبه همان
   shippingResolveCost() صفحهٔ تسویه است، پس رقمِ سبد با رقمِ سفارش یکی است.
...
/* «جمعِ کالاها» که به shippingCartSummary داده می‌شود شاملِ مالیات هم هست تا
   payable برگشتی (کالا + ارسال) درست باشد؛ ردیفِ نمایشیِ «جمع کالاها» پایین‌تر
   همچنان از $cartTotal خالص می‌آید، مالیات ردیفِ جداگانهٔ خودش را دارد. */
...
/* ---------- مرحلهٔ «بررسی عکس نمونهٔ قطعه» ----------
   اگر مدیر این مرحله را روشن گذاشته باشد، کلیدِ «ادامه» به‌جای صفحهٔ ثبت سفارش
...
<div class="container">
            ...
            <tr class="cart-row" data-id="<?= $p['id'] ?>">
                ...
                <td data-label="تعداد">
                    ...
                    <form method="POST" action="" class="cart-update-form" data-id="<?= $p['id'] ?>"
                        ...
                        <?php /* دکمهٔ پیش‌فرضِ فرم: تا Enter زدن داخل فیلد، همان تعدادِ
    ...
    <?php if ($shipSum): ?>
    <?php /* انتخاب روش ارسال + برآورد هزینه. هزینهٔ روشِ انتخاب‌شده در همین صفحه
            به مبلغ فاکتور اضافه می‌شود و مشتری با انتخابِ ثبت‌شده به صفحهٔ ثبت
            سفارش می‌رود. «پیک مشهد» همیشه در فهرست است و پنهان نمی‌شود، ولی اگر
            شهرِ پروفایل خوانده شود و مشهد نباشد، ردیفش کم‌رنگ و رادیویش disabled
            می‌شود (خواستهٔ مدیر: «غیر فعال نشون داده بشه») — شهرِ خالی هیچ روشی
    ...
    <div class="cart-ship" id="cart-ship">
        ...
        <?php /* روشِ محدود به یک شهر که با شهرِ پروفایل نمی‌خواند غیرفعال است؛ این
    ...
    <div class="cart-summary">
        ...
        <?php /* مبلغ قابل پرداخت = جمع کالاها + هزینهٔ روشِ انتخاب‌شده. رنگِ جداگانه
