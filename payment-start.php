...
if (!paymentReady()) {
    ...
    if (!$order) {
        ...
        /* قبلاً پرداخت شده — دوباره به درگاه نمی‌رویم */
        redirect('order-success.php?id=' . $orderId);
    } elseif ($order['status'] === 'cancelled') {
        $fail = 'این سفارش لغو شده و قابل پرداخت نیست.';
    } elseif (!orderPaymentUnlocked($order)) {
        /* گیتِ «بررسی موجودی» — حتی اگر مشتری این آدرس را مستقیم بزند (مثلاً
           با دکمهٔ برگشتِ مرورگر)، تا مدیر موجودی را تأیید نکند به درگاه
           نمی‌رود. */
        ...
        /* اگر روش ثبت‌شده آنلاین نیست (مثلاً پرداخت در محل)، درگاه فعال فعلی استفاده می‌شود.
           «پرداخت اعتباری» جدا سنجیده می‌شود: تنظیم‌شده‌بودنش کافی نیست و تیکِ
           pay_enable_credit هم باید روشن باشد، وگرنه سفارش به درگاه بانکیِ عادی می‌رود. */
        $gwOk = paymentIsOnline($gw) && paymentIsConfigured($gw)
                && ($gw !== 'credit' || paymentCreditEnabled());
        if (!$gwOk) $gw = paymentActiveGateway();

        if ($gw === '') {
            $fail = 'در حال حاضر هیچ درگاه پرداخت آنلاینی فعال نیست. لطفاً با ما تماس بگیرید.';
        } else {
