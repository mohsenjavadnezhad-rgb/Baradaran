<?php
/* مهاجرت «تحویل به پست + پیامک مراحل + درگاه اعتباری» — یک‌بار اجرا.
   ---------------------------------------------------------------
   سه چیز اضافه می‌شود:
   1) orders.track_post_at — مرحلهٔ تازهٔ «تحویل به پست» در روند ارسال. بین
      ...
      را که واقعا انجام شده تیک بزند.
   2) orders.post_tracking_code — کد رهگیری مرسولهٔ پست. ادمین با تیک‌زدن
      «تحویل به پست» کادرش را می‌بیند و کد را وارد می‌کند؛ مشتری همان کد را
      در تایم‌لاین می‌بیند.
   3) کلیدهای settings برای اطلاع‌رسانی پیامکی مراحل و درگاه اعتباری. همه با
      INSERT IGNORE بذر می‌شوند، پس اجرای دوباره چیزی را بازنویسی نمی‌کند.
      پیش‌فرض‌ها: پیامک مراحل روشن (sms_track_enabled=1) اما لایهٔ پیامک خودش
...
/* ---- ۲) بذر کلیدهای تنظیمات ---- */
echo '<h3>کلیدهای تنظیمات</h3>';
$seed = [
    /* پیامک مراحل ارسال (بخش ۵) */
    'sms_track_enabled'      => '1',
...
try {
    ...
    $ok++;
    echo "✅ بذر تنظیمات — $added کلید تازه اضافه شد (کلیدهای موجود دست‌نخورده)<br>";
} catch (Exception $e) {
    $fail++;
    echo "❌ بذر تنظیمات — " . htmlspecialchars($e->getMessage()) . "<br>";
}
...
echo '<hr><p>اگر همه ok بود، این فایل کارش تمام است. برای بستن دسترسی، محتوایش را با
