...
/* ==========================================================================
   ...
   منبع هر بنر به‌ترتیب اولویت:
     ۱) تصویر و لینکِ ثبت‌شده در پنل ادمین (جدول banners)
     ۲) فایل هم‌نام در uploads/banners/  (main.jpg, wide1.jpg, box1.jpg …)
     ۳) جای‌گیرِ خاکستری با ابعاد پیشنهادی (وقتی هیچ تصویری نباشد)
...
/* بنر «تخفیف ویژه» — زنده از محصولی که در ادمین تیکِ فروش ویژه خورده.
   کارتِ سمت راستِ ردیفِ زیر بنر اصلی (در RTL اولین فرزند سمت راست است). */
...
/* یک اسلاید از «آفر زمان‌دار»: تصویر + قیمتِ تخفیف‌دار + شمارش معکوس +
   نوارِ زمانِ باقی‌مانده. مقدار اولیهٔ شمارنده و نوار در سرور رندر می‌شود
   تا بدون JS هم چیزی دیده شود.
   آفرِ تمام‌شده (مهلت گذشته یا فروخته‌شده) از صفحه برداشته نمی‌شود؛ بنرش با
   نوشتهٔ «مهلت خرید تمام شد» و در صورت فروش، مهرِ «فروخته شد» در گوشهٔ چپِ بالا
   می‌ماند و کلیدِ خرید به «مشاهدهٔ محصول» تبدیل می‌شود.
   نکته: برچسبِ متنیِ «آفر زمان‌دار» به‌درخواست کاربر روی بنر نوشته نمی‌شود. */
function renderOfferBanner($o, $active = true) {
    ...
    /* درصدِ زمانِ باقی‌مانده برای نوار — فقط وقتی بازهٔ آفر معلوم باشد */
    ...
    $overNow = $sold || $expired;   /* آفر دیگر قابلِ خرید نیست */
    ...
    $out .= $img
          ...
          : '<span class="pc-noimg">' . icon('image') . '</span>';
    if ($has) $out .= '<span class="pc-off">' . $off . '٪ تخفیف</span>';
    /* مهرِ «فروخته شد» — گوشهٔ چپِ بالای تصویر (در RTL یعنی inset-inline-end) */
    ...
    if ($endTs) {
        ...
        /* بدونِ data-end، شمارندهٔ آفرِ تمام‌شده در جاوااسکریپت ردیابی نمی‌شود
        ...
        /* آفرِ بی‌زمان که فروخته شده: بی‌شمارنده هم باید پیام تمام‌شدن را ببیند */
...
/* اسلایدرِ آفرها: همهٔ آفرهای فعال روی هم قرار می‌گیرند و فقط اسلاید فعال
...
/* آفرها را به‌صورت گردشی بین اسلات‌های موجود پخش می‌کند تا هر اسلات
   اسلایدرِ مستقلِ خودش را داشته باشد (۵ آفر و ۳ اسلات ⇒ ۲/۲/۱). */
...
/* اسلاتِ خالیِ ردیف — کادرِ خط‌چین، همان زبانِ بقیهٔ اسلات‌های این صفحه.
   ردیف همیشه سه‌تایی می‌ماند تا جمعِ عرض کارت‌ها اندازهٔ بنر اصلی بشود.
   راهنمای پرکردن فقط برای مدیرِ واردشده نوشته می‌شود، نه برای مشتری. */
...
/* ---------- ردیفِ سه‌کارتیِ زیرِ بنر اصلی ----------
   همیشه سه اسلات رندر می‌شود تا جمعِ عرضشان دقیقاً اندازهٔ بنر اصلی باشد.
   اگر محصولی تیکِ «فروش ویژه» داشته باشد، اسلاتِ اول از آنِ اوست و آفرهای
   زمان‌دار بین اسلات‌های باقی‌مانده پخش می‌شوند؛ اسلاتِ بی‌محتوا خط‌چین می‌شود. */
$promoSlots = 3;
$promoCards = [];
...
<div class="container banners-page">
    <div class="banners-wrap">
        ...
        <!-- بنر اصلی: تمام‌عرضِ کانتینر و دست‌نخورده -->
        <?= renderBanner('main', 1200, 420, 'بنر اصلی', $mainImg, $mainHref) ?>

        <!-- سه بنرِ مجزا زیرِ بنر اصلی: «تخفیف ویژه» (اگر محصولی تیک داشته
             باشد) و آفرهای زمان‌دار. مجموعِ عرضِ سه کارت دقیقاً به اندازهٔ
             بنر اصلی است و فاصلهٔ بین‌شان آن‌ها را از هم جدا نگه می‌دارد.
             ردیف همیشه سه‌تایی رندر می‌شود؛ اسلاتِ بی‌محتوا خط‌چین می‌ماند. -->
        <div class="promo-row">
...
<?php if ($offers): ?>
<script>
/* آفرهای زمان‌دار: شمارش معکوسِ همهٔ اسلایدها + چرخشِ خودکارِ اسلایدر.
   بدون کتابخانه؛ مقصدِ هر شمارنده به‌صورت epoch در data-end است. */
(function () {
    ...
    function tick() {
        ...
        for (var i = 0; i < counts.length; i++) {
            ...
            if (left <= 0) {
                ...
                /* برچسبِ «زمان باقی‌مانده» پیش از شمارنده است و با انتخابگرِ
                   خواهرِ CSS قابلِ دسترسی نیست؛ پس همین‌جا پنهان می‌شود. */
            ...
            /* نوار زمان: نسبتِ زمانِ باقی‌مانده به کلِ بازهٔ آفر */
    ...
    /* ---------- چرخشِ خودکارِ اسلایدرها ----------
       هر اسلاتِ ردیف می‌تواند اسلایدرِ مستقلِ خودش باشد، پس روی همهٔ
       اسلایدرها حلقه می‌زنیم و هرکدام تایمر و وضعیتِ جدا می‌گیرد.
       اسلایدرِ تک‌اسلایدی نادیده گرفته می‌شود (چیزی برای چرخاندن ندارد). */
    ...
    function initSlider(sl) {
        ...
        /* با نگه‌داشتن موس روی اسلایدر چرخش می‌ایستد تا کاربر فرصت خواندن داشته
           باشد؛ در تبِ پنهان هم بی‌جهت نمی‌چرخد. */
