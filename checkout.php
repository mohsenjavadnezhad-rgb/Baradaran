...
/* ---------- گاردِ مرحلهٔ «بررسی عکس نمونهٔ قطعه» ----------
   اگر این مرحله روشن است و مشتری نه عکسِ تأییدشده دارد و نه مرحله را رد کرده،
   به part-check.php برمی‌گردد. فقط روی GET؛ چون اگر POSTِ ثبتِ نهایی هم ریدایرکت
   شود، مشتری‌ای که همین حالا مرحله را گذرانده ولی نشستش لب‌مرزی است سفارشش را
   از دست می‌دهد. */
if ($_SERVER['REQUEST_METHOD'] === 'GET' && !partCheckPassed($cartItems, $c)) {
    redirect('part-check.php');
}

/* روش‌های ارسال فعال. انتخاب فقط از صفحهٔ سبد خرید می‌آید (در نشستِ سرور ذخیره
...
/* ---------- نرخ‌نامه: شهر مقصد + وزن ⇒ هزینهٔ خودکار ----------
   ...
   بی‌آنکه چیزی بنویسد نرخِ تبریز را می‌بیند) و وزن از مجموع وزنِ کالاهای سبد،
   ...
   نداشته باشد یک واحد حساب می‌شود. جملهٔ توضیحیِ نرخ («نرخ فلان‌شهر: هر ۱
   کیلوگرم …») این‌جا نمایش داده نمی‌شود (خواستهٔ مدیر) — تنها خودِ رقم می‌آید. */
$shipCityIn = trim((string)($_POST['city'] ?? $c['city'] ?? ''));
$shipWeight = shippingCartWeight($cartItems)[0];
$shipTexts  = shippingRateTexts();
/* فقط نرخِ همان روشِ انتخاب‌شده به جاوااسکریپت می‌رود: با عوض‌شدن شهر باید رقمِ
...
/* هزینهٔ ارسالِ انتخاب فعلی. برای روش‌های «پس‌کرایه» هیچ محاسبه‌ای انجام
...
/* روش‌های پرداخت فعال (از تنظیمات ادمین). پس از محاسبهٔ «مبلغ قابل پرداخت»
   خوانده می‌شود چون پرداخت اعتباری می‌تواند حدِ پایینِ مبلغ داشته باشد
...
/* هزینه‌های باربری (بخش جدای تنظیمات) — فقط زیر روشِ باربری نشان داده می‌شود */
...
/* قاعدهٔ ارسال↔پرداخت (shippingAllowedPayKeys): «پرداخت در محل» فقط برای روشی
   که تیکِ آن را دارد (ارسال با پیک) مجاز است — و روی همان روش، پرداخت اینترنتی
   و کارت‌به‌کارت هم باز است، چون ممکن است مشتریِ مشهدی آنلاین پرداخت کند.
   گزینه‌های غیرمجاز در فرم disabled می‌شوند (پس اصلاً ارسال نمی‌شوند) و در سرور
...
/* ---------- کارت به کارت: ورودی‌های واریز روی همین صفحه ----------
   ...
   می‌شوند. مبلغ پیش‌فرض همان مبلغ قابل پرداخت و تاریخ و ساعتِ پیش‌فرض همین
   لحظه است، چون واریز معمولاً همان لحظه انجام می‌شود؛ همه قابل ویرایش‌اند.
   تاریخ از تقویم شمسیِ همین صفحه انتخاب می‌شود و ساعت از دو انتخابگرِ «ساعت»
   و «دقیقه» (خواستهٔ مدیر: «تاریخ رو از روی تقویم فارسی بهم بده / و زمان رو هم
   با یک ساعت و دقیقه قابل انتخاب»).
   منطقهٔ زمانی صریحاً تهران گرفته می‌شود: منطقهٔ زمانیِ سرور تنظیم نشده و
   پیش‌فرضِ ساعت نباید چند ساعت عقب‌تر از ساعتِ واقعیِ مشتری باشد. */
...
/* تاریخ: ارقام فارسی به لاتین و یکدست‌کردن جداکننده. مشتریِ بی‌جاوااسکریپت که
...
/* ساعت و دقیقه دو انتخابگرِ جدا هستند (پس بدون جاوااسکریپت هم کار می‌کنند) و
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_order'])) {
    ...
    $mobile   = $c['mobile']; // شمارهٔ ثابتِ حساب کاربری
    $province = trim($_POST['province'] ?? '');
    /* نامِ رسمیِ شهر از فهرست شهرها؛ اگر نخواند همان چیزی که مشتری نوشته
       می‌ماند. یکدست‌شدنِ نام باعث می‌شود نرخ‌نامه و صفحهٔ سبدِ خریدِ بعدی هم
       همین شهر را بشناسند. */
    ...
    /* روش ارسال هم فقط از میان روش‌های فعال؛ هزینه در سرور از نرخ‌نامه/تنظیمات
       ...
       شهر و وزن همان چیزی است که ثبت می‌شود، پس رقمِ سفارش با رقمی که مشتری
       دیده یکی است. */
    ...
    /* قاعدهٔ ارسال↔پرداخت دوباره در سرور بررسی می‌شود: گزینه‌های غیرمجاز در فرم
       disabled هستند، ولی بدون جاوااسکریپت یا با فرمِ دست‌کاری‌شده هم نباید
       ترکیب نامعتبر ثبت شود (مثلاً پرداخت در محل برای ارسال پستی). */
    if ($shipMethod !== '' && !in_array($payMethod, shippingAllowedPayKeys($shipMethod, $payKeys), true)) {
        $errors[] = shippingPayRuleNote($shipMethod) . ' لطفاً روش پرداخت را اصلاح کنید.';
    }

    /* روشِ محدود به یک شهر (پیکِ مشهد) با شهرِ همین فرم هم سنجیده می‌شود: شهرِ
       تحویل روی این صفحه قابل تغییر است و پیکِ شهری نباید به شهر دیگری برود.
       شهرِ خالی خطا نمی‌دهد (همان قاعدهٔ «نمی‌دانیم یعنی ممنوع نیست»). */
    ...
    if (empty($errors)) {
        try {
            ...
            /* ستون‌های مالیات روی order_items هم گارد می‌شوند تا بین آپلودِ کد و
               اجرای مهاجرت، ثبت سفارش 500 نگیرد — چیزی که بدونِ این گارد ممکن بود
               رخ بدهد چون این ستون‌ها تازه‌اند (طبق قاعدهٔ ترتیبِ دیپلوی). */
            ...
            /* اطلاعات واریزِ کارت‌به‌کارت همراه خودِ سفارش ثبت می‌شود و وضعیت
            ...
            /* درخواستِ بررسیِ عکس (اگر بود) به همین سفارش گره می‌خورد تا ادمین آن
            ...
            /* «بررسی موجودی» اولین مرحلهٔ روند ارسال است و تا ثبت نشود هیچ
               پرداختی — حتی درگاهِ بانکی — باز نمی‌شود (خواستهٔ مدیر). اگر همین
               سفارش از «بررسی عکس نمونهٔ قطعه» با موجودیِ تأییدشده رد شده
               باشد، همین‌جا خودکار طی می‌شود؛ وگرنه مدیر باید در جزئیاتِ
            ...
            /* پرداخت آنلاین: مشتری به درگاه منتقل می‌شود — فقط اگر گیتِ موجودی باز باشد؛
               وگرنه به صفحهٔ نتیجه می‌رود که پیامِ «در انتظار بررسی موجودی» را
               نشان می‌دهد و دکمهٔ پرداخت را تا باز شدنِ گیت پنهان نگه می‌دارد.
               پرداخت در محل/کارت‌به‌کارت/چک همیشه مستقیم به همان صفحه می‌روند —
               این‌ها «پرداختِ آنی» نیستند، بلکه اطلاعاتی‌اند که مدیر بعداً
...
<div class="container">
    ...
    <form method="POST" class="checkout-form">
        <?php /* data-goods شاملِ مالیات هم می‌شود (نه فقط جمعِ کالاها) چون جاوااسکریپتِ
                زیرِ همین صفحه با فرمولِ «goods + هزینهٔ ارسال» مبلغِ قابل‌پرداخت را
                زنده حساب می‌کند و مالیات با تغییرِ شهر/روشِ ارسال عوض نمی‌شود؛
                ردیفِ «جمع کالاها»ی زیر همچنان فقط جمعِ خالصِ کالاها را نشان می‌دهد. */ ?>
        <div class="checkout-summary" data-goods="<?= (int)($cartTotal + $taxTotal) ?>">
            ...
            <?php endif; ?>
            <?php if ($shipMethods): ?>
            <?php /* روش ارسال در سبد انتخاب شده؛ این‌جا فقط نامش را با یک لینکِ
        ...
        <?php /* چیدمان دوستونه (خواستهٔ کاربر): مشخصات و آدرس سمت راست، روش
                ارسال و پرداخت سمت چپ. در RTL اولین فرزندِ گرید سمت راست
        ...
        <div class="checkout-cols">
            ...
            <div class="checkout-col-side">
                ...
                <?php /* «هزینه‌های باربری» از تنظیمات ادمین — فقط اطلاع‌رسانی و فقط
                        وقتی روش ارسالِ انتخاب‌شده باربری است؛ مبلغی که به سفارش
                        اضافه می‌شود همان هزینهٔ خودِ روش است. */ ?>
                ...
                <?php /* قاعدهٔ ارسال↔پرداخت: گزینه‌های ناسازگار با روش ارسالی که در
                        ...
                        همین حالتِ رندرشدهٔ سرور نهایی است و اسکریپتی آن را عوض
                ...
                <div class="pay-picker" id="pay-picker">
                    ...
                    <?php /* پیامِ قاعدهٔ پرداخت («برای … پرداخت در محل امکان‌پذیر نیست»)
                            برداشته شد — تصمیم مدیر: خودِ گزینهٔ کم‌رنگ و غیرفعال کافی
                            است. جایش، فقط برای روش‌های پس‌کرایه، توضیحِ واقعاً مفیدی
                    ...
                    <label class="pay-opt <?= $payDefault === $pk ? 'is-on' : '' ?> <?= $pOff ? 'is-off' : '' ?>">
                        ...
                        <span class="pay-opt-body">
                            ...
                            <?php elseif ($pk === 'card'): ?>
                            <?php /* کارت به کارت: شمارهٔ کارتِ مقصد همین‌جا نشان داده می‌شود تا
                            ...
                            <?php /* پرداخت اعتباری/اقساطی: متنِ توضیح از تنظیمات می‌آید چون نام و
                                    شرایطِ ارائه‌دهندهٔ اعتبار متفاوت است. اگر مدیر متن را خالی
                            ...
                            <?php /* ویژهٔ همکارِ تأییدشده — چیزی اینجا گرفته نمی‌شود؛ سفارش
                    ...
                    <?php /* اطلاعات واریز، فقط وقتی «کارت به کارت» انتخاب شده باشد.
                            هیچ فیلدی required نیست: کادر پنهان می‌شود و فیلدِ
                            پنهانِ required جلوی ارسال فرم را بی‌آنکه دیده شود
                        ...
                        <div class="pc2-grid">
                            <div class="form-group">
                                ...
                                <input type="text" name="c2c_ref" id="c2c_ref" class="form-control" dir="ltr"
                                       value="<?= h($c2cIn['ref']) ?>" placeholder="مثلاً 123456789">
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <?php /* تقویم شمسی (خواستهٔ مدیر: «تاریخ رو از روی تقویم فارسی
                                        بهم بده»). خودِ ورودی همچنان قابل تایپ است تا بدون
                                        جاوااسکریپت هم پر شود؛ تقویم فقط کارِ مشتری را راحت
                                        می‌کند و همان قالبِ 1405/06/03 را می‌نویسد — همان چیزی
                            ...
                            <div class="form-group">
                                ...
                                <?php /* ساعت و دقیقهٔ انتخابی (خواستهٔ مدیر: «زمان رو هم با یک
                                        ساعت و دقیقه قابل انتخاب»). دو انتخابگرِ ساده‌اند تا
        ...
        /* این صفحه فقط ثبت نهایی است: روش ارسال در سبد انتخاب شده و این‌جا
           عوض نمی‌شود. کارِ اسکریپت سه چیز است — هایلایت گزینهٔ پرداخت،
        ...
        (function(){
            ...
            /* متن‌ها و نرخ‌نامه از سرور می‌آیند تا ارقامِ زنده با آنچه سرور رندر
               ...
               لازم است فرستاده می‌شود؛ قالبِ جملهٔ توضیحیِ نرخ‌نامه حتی به‌صورت
            ...
            /* شهرِ انتخاب‌شده که عوض می‌شود، نرخِ همان روش هم عوض می‌شود. رخداد
               change را خودِ انتخابگرِ استان هم پس از بازسازیِ فهرستِ شهر شلیک
            ...
            /* کادرهای اطلاعاتِ واریز/چک فقط با انتخابِ همان روشِ پرداخت باز
               می‌شوند. required را هم‌زمان می‌گذاریم و برمی‌داریم، چون فیلدِ
               پنهانِ required ارسال فرم را بی‌آنکه پیامی دیده شود متوقف می‌کند —
            ...
            /* نیم‌فاصله/ZWJ/کشیده با کد ساخته می‌شوند چون دیده نمی‌شوند و ویرایشِ
               بعدیِ همین خط را خطرناک می‌کردند. */
            ...
            /* تعداد واحدهای وزنی — مثل shippingRateUnits(): همیشه رند به بالا و
               حداقل یک واحد. ۰٫۰۰۰۰۱ خطای شناوری را می‌گیرد تا ۳ کیلوِ دقیق با
            ...
            /* همان تصمیم‌گیریِ shippingResolveCost() سرور:
               پس‌کرایه ⇒ هیچ محاسبه‌ای؛ وگرنه نرخِ شهر × تعداد واحدهای وزنی. */
            function resolve(key, city) {
                ...
                if (nc !== '') {
                    ...
                    /* برابریِ کامل مقدم است (کسی که «قم» می‌نویسد نرخِ «قمصر» را
            ...
            /* جمع زدنِ زندهٔ هزینهٔ ارسال با مبلغ کالاها. رقم از همان resolve()
               می‌آید که نسخهٔ جاوااسکریپتیِ shippingResolveCost() سرور است، پس
               «هزینهٔ ارسال» و «مبلغ قابل پرداخت» هیچ‌وقت با هم اختلاف پیدا
               نمی‌کنند. مبلغِ نهایی هنگام ثبت سفارش دوباره در سرور حساب می‌شود؛
               این فقط نمایش است. */
        ...
        <?php if ($c2cReady): ?>
        <script>
        /* ---------- تقویم شمسیِ «تاریخ واریز» ----------
           خواستهٔ مدیر: «تاریخ رو از روی تقویم فارسی بهم بده». بدون هیچ
           کتابخانه‌ای؛ یک کادرِ بازشو که همان قالبِ 1405/06/03 را در ورودی
           می‌نویسد — هم‌شکلِ خروجیِ jDate() سرور، تا رقم‌های ادمین یکدست
        ...
        (function(){
            ...
            /* دو تبدیل زیر همان الگوریتمِ gregorianToJalali() سرور هستند، پس
            ...
            /* شمارِ روزهای ماه: شش ماه اول ۳۱، پنج ماه بعد ۳۰، اسفند ۲۹ یا ۳۰.
               کبیسه‌بودن با رفت‌وبرگشتِ «۳۰ اسفند» سنجیده می‌شود تا با همین دو
            ...
            /* تایپِ دستی و تقویم هم‌گام می‌مانند */
