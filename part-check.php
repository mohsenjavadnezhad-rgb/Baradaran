<?php
/* گام ۲ از ۴ (سبد → بررسی عکس → بررسی موجودی → ثبت سفارش): «ارسال عکس
   نمونهٔ قطعه». مشتری اینجا آپلود می‌کند یا رد می‌کند و به stock-check.php
   (گام ۳) می‌رود تا در نوبت ادمین بماند. اما این صفحه خودش را کاملا بسته
   نمی‌کند: تا وقتی نتیجه «approved» نشده، مشتری می‌تواند دوباره به همین‌جا
   برگردد و نظرش را عوض کند (مثلا بعد از «رد کردن»، بالأخره عکس بفرستد) —
   ثبت دوبارهٔ عکس همان درخواست در-انتظار را جایگزین می‌کند، ردیف تازه‌ای
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    /* رد کردن مرحله — از ۲۰۲۶-۰۸-۳۰ دیگر بایپس بی‌درنگ نیست (خواستهٔ کاربر:
       ...
       و مشتری به stock-check.php (گام ۳) می‌رود تا در صف ادمین بماند؛
       کارشناس دیگر مطابقت عکس را نمی‌سنجد (چیزی نفرستاده)، فقط موجودی را
       تأیید می‌کند. */
    if ($act === 'skip') {
...
/* وضعیت فعلی. تنها حالتی که از این صفحه بی‌قیدوشرط بیرون می‌رود «approved»
   است — آن‌جا دیگر واقعا چیزی برای تغییر نمانده، نتیجه در stock-check.php
   ...
   میپره به بررسی موجودی»). حالا وقتی pending است هم فرم آپلود و هم جعبهٔ
   ردکردن این‌جا می‌مانند؛ ثبت دوبارهٔ عکس همان ردیف در-انتظار را جایگزین
   می‌کند (بالاتر)، نه یک ردیف تکراری. */
...
<div class="container">
    ...
    <?php /* چهار‌گام خرید — مشتری بداند کجای مسیر است. دو گام اول (سبد و
            همین صفحه) قابل کلیک‌اند تا مشتری بتواند عقب برود؛ دو گام بعدی
            صرفا نشانگرند (بی‌آنکه از گیت رد شده باشد، رفتن به آن‌ها بی‌معناست
            و برمی‌گردد). */ ?>
    <ol class="pchk-steps">
    ...
    <div class="flash flash-success">
        ...
        می‌توانید صبر کنید (<a href="stock-check.php">پیگیری وضعیت</a>) یا اگر نظرتان عوض شده، همین‌جا عکس تازه بفرستید — جایگزین درخواست قبلی می‌شود.
    </div>
    <?php endif; ?>
